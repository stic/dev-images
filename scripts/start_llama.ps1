...
# docker run --rm -v llama-models:/models -v /host/path/to/model:/model.gguf alpine cp /model.gguf /models/
...
docker run -d --name llama-server --gpus all -p 6060:6060 -v llama-models:/models `
  ...
  llama-server -m $MODEL_PATH --host 0.0.0.0 --port 6060 --ctx-size 4096 --threads $env:NUMBER_OF_PROCESSORS
